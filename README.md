...
第一著者として、国際英文学術誌に発表した研究成果を、公開データセット参照、再利用可能なYOLOv5_FPC検出スクリプト、朝鮮日報ニュースライブラリの公開サービス、書籍/IPビジネス成果へ接続したAIデータサイエンスプロジェクトです。  
KAISTとChosun Ilbo Media InstituteによるR&D産業連携として、新聞アーカイブに埋もれた四コマ漫画を検出・構造化し、検索可能なデータ資産として活用できる基盤を構築しました。  
曖昧なメディアアーカイブ課題を、データ収集、ラベリング、YOLOv5_FPC開発、AIモデル評価、大規模推論、DB化、公開サービス価値創出までの実行可能なワークフローへ具体化しました。
